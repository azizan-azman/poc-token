...
const tokenFilter = (category, type) => (token) => {
  return category === "global"
    ? token.attributes.type === type && token.attributes.category === category
  ...
  return tokenSetTypes.map((type) => ({
    ...
    destination:
      theme === "global"
        ...
        : `my-build2/themes/${theme}/_theme.${theme.toLowerCase()}.scss`,
...
async function run() {
  const $tokenSetOrder = JSON.parse(
    await promises.readFile(
      "tokens-example/free/single-file/cti-long-item-name.tokens.json"
    )
  );
