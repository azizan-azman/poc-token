...
const tokenFilter = (type, theme) => (token) => {
  const filePath = `tokens-example/free/multi-file/new-cti/${theme}.json`;
  return token.attributes.type === type && token.filePath === filePath;
};
...
const generateFilesArr = (data, theme) => {
  ...
  return tokenSetTypes.map((type) => ({
    ...
    destination:
      theme === "global"
        ? `my-build4/${theme}/_${theme.toLowerCase()}.${type}.scss`
        : `my-build4/themes/${theme}/_${theme.toLowerCase()}.${type}.scss`,
...
async function readTokenFile(tokenType) {
  const filePath = `tokens-example/free/multi-file/new-cti/${tokenType}.json`;
  try {
...
async function run() {
  const $tokenSetOrder = JSON.parse(
    await promises.readFile(
      "tokens-example/free/multi-file/new-cti/$metadata.json"
    )
  );
  const myconfigsPromises = $tokenSetOrder.tokenSetOrder.map(
    async (tokenSet) => {
      ...
      return {
        source:
          tokenSet === "global"
            ? [`tokens-example/free/multi-file/new-cti/global.json`]
            : [
                `tokens-example/free/multi-file/new-cti/global.json`,
                `tokens-example/free/multi-file/new-cti/${tokenSet}.json`,
              ],
        fileHeader: {
